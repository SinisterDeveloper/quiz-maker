		...
		return (
			...
			>
				...
				>
					...
					{loginError ?
						<p className="text-red-300 mt-2">{loginError}</p>
					:	null}
				</form>
			</div>
		);
	...
	return (
		...
		>
			...
			<div className="flex-grow p-4 flex-1 flex">
				<div
					className={`webpage bg-zinc-900 p-6 rounded-lg shadow-lg max-w-screen flex-grow ${activeTab === 'addquestion-option' ? 'block' : 'hidden'}`}
					id="addquestion-option"
				>
					<h1 className="text-xl font-bold mb-4">Add Question</h1>
					{selectedDeckForQuestion ?
						<p className="text-blue-300 mb-4">
							Deck ID: {selectedDeckForQuestion}
						</p>
					:	<p className="text-yellow-400 mb-4">
							Open a deck and click "Add Question" first.
						</p>
					}
					<form className="space-y-4" onSubmit={handleQuestionCreate}>
						<div className="flex items-center">
							<label className="w-1/3 text-blue-200">
								Question
							</label>
							<textarea
						...
						{questionStatus ?
							<div className="flex justify-center mt-2">
								<p
									className={`mt-4 p-2 bg-zinc-700 rounded ${questionStatus.startsWith('Error:') ? 'text-red-400' : 'text-green-400'}`}
								>
									{questionStatus}
								</p>
							</div>
				...
				<div
					className={`webpage bg-zinc-900 p-6 rounded-lg shadow-lg max-w-screen flex-grow ${activeTab === 'adddisplay-option' ? 'block' : 'hidden'}`}
					...
					{displayQuestionData ?
						<div className="space-y-4">
							<div className="flex items-center">
								...
								<textarea
									className="mb-3 w-2/3 p-2 border rounded bg-zinc-700 text-zinc-100"
									value={displayQuestionData.question}
									disabled
								/>
							</div>
							{displayQuestionData.options.map(
								(option, index) => (
									<div
										className="flex items-center"
										key={option + index}
							...
							<div className="flex items-center">
								...
								<input
									className="mb-3 w-2/3 p-2 border rounded bg-zinc-700 text-zinc-100"
									value={displayQuestionData.points}
									disabled
								/>
							</div>
							<div className="flex items-center">
							...
							<div className="flex justify-center mt-4">
								...
								<button
									...
									onClick={() =>
										deleteQuestion(
											displayQuestionData.id,
											displayQuestionData.deck,
										)
									}
								>
									Delete
				...
				<div
					className={`webpage rounded-lg shadow-lg max-w-screen flex-grow ${activeTab === 'viewquestions-option' ? 'block' : 'hidden'}`}
					...
					<div className="flex justify-between items-center mb-4">
						...
						<div className="flex items-center space-x-2">
							<button
								...
								disabled={viewQuestionPage === 0}
								onClick={() =>
									setViewQuestionPage((page) =>
										Math.max(page - 1, 0),
									)
								}
							>
							...
							<span id="q-page-count">
								Page{' '}
								{questionPages.length ?
									viewQuestionPage + 1
								:	1}{' '}
								of {questionPages.length || 1}
							</span>
							<button
								id="next-q"
								className="bg-green-500 p-2 rounded"
								disabled={
									viewQuestionPage + 1 >= questionPages.length
								}
								onClick={() =>
									setViewQuestionPage((page) =>
										Math.min(
				...
				<div
					className={`webpage rounded-lg shadow-lg max-w-screen inset-0 ${activeTab === 'help-container' ? 'block' : 'hidden'}`}
					id="help-container"
				>
					<p className="mb-2">
						Instructions on how to create and share a quiz can be
						found{' '}
						<a
					...
					<p className="my-4">
						...
						<em>
							without modification (resizing, compression, etc.)
						</em>{' '}
						to its storage and serves the image file as is to the
						quiz page.
					</p>
					<h1 className="text-2xl font-bold my-4">Contact</h1>
					<p>
						You may email me (
						<code className="font-mono bg-gray-700 p-1 rounded">
							thesinisterdev@gmail.com
						</code>
						) or{' '}
						<em>
							<a
				...
				<div
					className={`webpage bg-zinc-900 p-6 rounded-lg shadow-lg max-w-screen flex-grow ${activeTab === 'adddeck-option' ? 'block' : 'hidden'}`}
					id="adddeck-option"
				>
					...
					<form className="space-y-4" onSubmit={handleDeckCreate}>
						...
						<div className="flex items-center">
							<label className="w-1/3 text-blue-200">
								Quiz Background Image (Optional, BETA)
							</label>
							<div className="flex space-x-2">
						...
						{deckStatus ?
							<div className="flex justify-center mt-2">
								<p
									className={`mt-4 p-2 bg-zinc-700 rounded ${deckStatus.startsWith('Error:') ? 'text-red-400' : 'text-green-400'}`}
								>
									{deckStatus}
								</p>
							</div>
				...
				<div
					className={`webpage rounded-lg shadow-lg max-w-screen flex-grow ${activeTab === 'deck-container' ? 'block' : 'hidden'}`}
					id="deck-container"
				>
					<div
					...
					>
						...
						<div className="flex items-center space-x-2">
							<button
								...
								disabled={deckPage === 0}
								onClick={() =>
									setDeckPage((page) => Math.max(page - 1, 0))
								}
							>
								◄
							</button>
							<span id="d-page-count">
								Page {deckPages.length ? deckPage + 1 : 1} of{' '}
								{deckPages.length || 1}
							</span>
							<button
								id="next-d"
						...
						<button
							className="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700"
							id="new-deck"
							onClick={() => openTab('adddeck-option')}
						>
							New Deck
						</button>
					</div>
					...
					>
						{deckPageItems.map((deck) => {
							...
							return (
								...
								>
									<p className="absolute top-2 right-2 bg-zinc-700 px-2 py-1 rounded">
										#{deck.id}
									</p>
									<div className="flex justify-between items-center mb-4">
										<p className="text-lg font-semibold text-blue-300 w-full">
											{deck.name}
										</p>
									</div>
				...
				<div
					className={`webpage rounded-lg shadow-lg max-w-screen flex-grow ${activeTab === 'result-container' ? 'block' : 'hidden'}`}
					id="result-container"
				>
					<div
					...
					>
						...
						<div className="flex items-center space-x-2">
							...
							<span id="r-page-count">
								Page {resultPages.length ? resultPage + 1 : 1}{' '}
								of {resultPages.length || 1}
							</span>
							<button
								id="next-r"
								className="bg-green-500 p-2 rounded"
								...
								onClick={() =>
									setResultPage((page) =>
										Math.min(
											page + 1,
											Math.max(resultPages.length - 1, 0),
										),
									)
								}
							>
						...
						{resultPageItems.map((result) => (
							...
							>
								...
								<div className="flex flex-col items-center text-blue-200">
									<p className="px-2 py-1 rounded w-full mb-2">
										Name: {result.name}
									</p>
									<p className="px-2 py-1 rounded w-full mb-2">
				...
				<div
					className={`webpage flex flex-col rounded-lg shadow-lg max-w-screen flex-grow items-center justify-center select-none ${activeTab === 'dashboard' ? 'flex' : 'hidden'}`}
					id="dashboard"
				>
					...
					<h1 className="text-center text-7xl font-bold my-8">
						Quiz-Maker
					</h1>
					<p className="text-center text-lg mb-8">
						Lightweight, AI-enhanced platform for effortless quiz
						creation and sharing with lightning-fast performance
					</p>
					<div className="flex space-x-2">
						<img
...
function QuizPage() {
	...
	const backgroundStyle =
		metadata?.background === 'custom' && deckId ?
			{
				backgroundImage: `url(/img/BG_${deckId}.jpeg)`,
	...
	return (
		<>
			...
			<div
				...
				style={{
					...backgroundStyle,
					display: submittedDetails ? 'flex' : 'none',
				}}
			>
				{showInstructions ?
					<div
						...
						<ul className="list-none p-0 space-y-4">
							<li>
								<h3 className="text-lg">
									Each question is in multiple-choice format
									with four options. Click on an option to
									save your answer.
								</h3>
							</li>
							<li>
				...
				:	<div
					...
					>
						...
						<h2 className="text-center text-xl mb-4" id="quiz-name">
							{metadata?.name || 'QUIZ NAME'}
						</h2>
						...
						<div className="flex justify-between text-sm">
							<span id="status">
								Question{' '}
								{questions.length ? questionIndex + 1 : 0} of{' '}
								{questions.length}
							</span>
							<span id="timer">
								{minutes}m {seconds}s
				...
				<div className="webpage text-white" id="result-page">
					<div style={{ textAlign: 'center' }}>
						...
						<table className="border-separate border-spacing-2 w-full border border-gray-500 bg-slate-900 text-sm shadow-sm">
							<tbody>
								<tr>
									<th className="bg-gray-700 w-1/6 border border-gray-600 font-semibold p-4 text-gray-200">
										Name
									</th>
									<td
						...
						<p id="conclusion" className="mt-3">
							{stats?.conclusion || ''}
						</p>
					</div>
				</div>
					...
					<div style={{ textAlign: 'center' }}>
						...
						<table className="border-separate border-spacing-2 w-full border border-gray-500 bg-slate-900 text-sm shadow-sm">
							<thead className="bg-gray-700">
								<tr>
									<th className="w-1/4 border border-gray-600 font-semibold p-4 text-gray-200">
										Question
									</th>
									<th className="w-1/4 border border-gray-600 font-semibold p-4 text-gray-200">
										Correct Answer
							...
							<tbody id="question-list">
								{answers.map((answer, index) => (
									<tr key={`${answer.question}-${index}`}>
										<td className="w-1/4 border border-gray-600 p-4 text-gray-200">
											{answer.question}
										</td>
										<td className="w-1/4 border border-gray-600 p-4 text-gray-200">
